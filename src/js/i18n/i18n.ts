...
// Supported languages
export const supportedLanguages = ['en', 'de', 'zh', 'vi', 'fr'] as const;
...
export const getLanguageFromUrl = (): SupportedLanguage => {
    ...
    const langMatch = path.match(/^\/(en|de|zh|vi|fr)(?:\/|$)/);
...
export const changeLanguage = (lang: SupportedLanguage): void => {
    ...
    let newPath: string;
    if (currentPath.match(/^\/(en|de|zh|vi|fr)\//)) {
        newPath = currentPath.replace(/^\/(en|de|zh|vi|fr)\//, `/${lang}/`);
    } else if (currentPath.match(/^\/(en|de|zh|vi|fr)$/)) {
        newPath = `/${lang}`;
    } else {
